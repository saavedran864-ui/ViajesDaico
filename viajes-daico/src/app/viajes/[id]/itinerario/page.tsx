...
export default function ItinerarioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [dias, setDias] = useState<(Dia & { actividades: Actividad[] })[]>([])
  ...
  async function load(id: string) {
    ...
    const diasConActs = (diasData ?? []).map(d => ({
      ...d,
      actividades: (acts ?? []).filter(a => a.dia_id === d.id),
    }))
    setDias(diasConActs)
    const exp: Record<string, boolean> = {}
    diasConActs.forEach(d => { exp[d.id] = true })
    setExpanded(exp)
    setLoading(false)
  }
  ...
  async function addDia(e: React.FormEvent) {
    ...
    const { data } = await supabase.from('dias').insert({
      viaje_id: viajeId, fecha: newDiaFecha, orden: dias.length,
    }).select().single()
    if (data) {
      setDias(d => [...d, { ...data, actividades: [] }].sort((a, b) => a.fecha.localeCompare(b.fecha)))
  ...
  async function addActividad(e: React.FormEvent, diaId: string) {
    ...
    const { data } = await supabase.from('actividades').insert({
      ...
      orden: dia?.actividades.length ?? 0,
    }).select().single()
    if (data) {
        ...
        <div key={dia.id} className="card">
          <div className="flex items-center gap-3 cursor-pointer"
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-nude-400">{dia.actividades.length} actividades</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={e => { e.stopPropagation(); deleteDia(dia.id) }}
                className="text-nude-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 p-1">
                <Trash2 size={13} />
              </button>
